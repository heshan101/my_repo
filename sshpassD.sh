...
EOF
  ...
  sshpass -f sshpass.txt ssh "${HOST1}" << EOF
    show interfaces ${host1} | match "Physical interface"
    show interfaces ${host1} | match "rate" | except "FEC"
    show interfaces ${host1} | match "error" | except "Link-level"
    show interfaces ${host1} | match "flap"
    show interfaces diagnostics optics ${host1} | except "Off|threshold|Module"
    show lldp neighbors interface ${host1} | match "System name|Port ID" | except "Local|Info"
    show interfaces ${host1}
EOF
  ...
  sshpass -f sshpass.txt ssh "${HOST1}" << EOF
    ...
    sh int ${PORT1}-4 transceiver dom thresholds
EOF
...
elif [[ "$HOST1" == *"fa"* ]] || [[ "${HOST1}" == *"rsw"* ]] || [[ "${HOST1}" == *"rtsw"* ]] || [[ "${HOST1}" == *"ssw"* ]] || [[ "${HOST1}" == *"fsw"* ]] ; then
...
EOF
  ...
  sshpass -f sshpass.txt ssh "${HOST2}" << EOF
    ...
    sh int ${PORT2}-4 transceiver dom thresholds
EOF
...
elif [[ "$HOST2" == *"fa"* ]] || [[ "${HOST2}" == *"rsw"* ]] || [[ "${HOST2}" == *"rtsw"* ]] || [[ "${HOST2}" == *"ssw"* ]] || [[ "${HOST2}" == *"fsw"* ]] ; then
